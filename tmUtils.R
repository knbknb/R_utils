...
tmUtil$tm_freqterms <- function(tdm, lowfreq = 1, topn = -1) {
  stopifnot(
    inherits(tdm, c("DocumentTermMatrix", "TermDocumentMatrix")),
...
tmUtil$tm_convertToUTF8 <- function(x) {
  doc <- tryCatch({
    ...
    warning(e)
    return("convertToUTF8(): cannot convert text")
  })
  PlainTextDocument(doc)
  # doc
}
...
tmUtil$tm_shown_meta <- function(corpus, ndoc = 1, tag = "*") {
  ...
  if (tag != "*") {
    sapply(randn, function(i) {
      meta(corpus[[i]], tag = tag)
    })
  } else {
    sapply(randn, function(i) {
      meta(corpus[[i]])
